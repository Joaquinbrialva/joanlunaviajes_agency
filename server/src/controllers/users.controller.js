...
const { SUCCESS_MESSAGES, AUTH_MESSAGES } = require('../utils/messages');
...
const getUserById = async (req, res, next) => {
	try {
		...
		return success(res, SUCCESS_MESSAGES.USER_FETCHED, user);
	} catch (error) {
		next(error);
	}
};
...
const updateUser = async (req, res, next) => {
	try {
		const { id } = req.params;
		const { name, lastName, email, password, phone, country, city, role } =
			req.body;
		const newData = {
			name,
			lastName,
		...
		return success(res, SUCCESS_MESSAGES.USER_UPDATED, userUpdated);
	} catch (error) {
		next(error);
	}
};
...
const deleteUser = async (req, res, next) => {
	try {
		...
		return success(res, SUCCESS_MESSAGES.USER_DELETED, { id });
	} catch (error) {
		next(error);
	}
};
